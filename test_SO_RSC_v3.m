function test_SO_RSC_v3(filename,initial_position,initial_heading, mode, visual_scene)
% This function tests a full SO model after simulation, to see how the vis
% to CONJ to HD weights allow the system to correct HD with inaccurate ADN
%input or track HD without ADN input.
...
%mode: 'inaccurate' = inaccurate ADN input
%      'absent' = absent ADN inputp

disp('Now testing landmark influence')
mode = mode(1); %making one letter to get the code running quicker
...
phi_ADNHD = 0; %testing not having ADN influence RSC
time = 10.0; % ten seconds of testing
...
if visual_scene
    ...
    [headings,positions] = circular_arena_walk_v3('SO_test',epochs/100, initial_position,initial_heading,sigma_head_turns, step_scale_lower,step_scale_upper);
    disp('Circular arena walk finished');
else
    sigma_head_turns = 25;
    step_scale_lower = 0.05;
    step_scale_upper = 0.05;
    [headings,positions] = one_compartment_walk_v2('SO_test',epochs/100, initial_position,initial_heading,sigma_head_turns, step_scale_lower,step_scale_upper);
...
%% Save test rates
fileID = fopen('visRates_test.bdat','wb');
fwrite(fileID,rates_vis_time,'float32');
fclose(fileID);

fileID = fopen('HDRates_test.bdat','wb');
fwrite(fileID,rates_HD_time,'float32');
fclose(fileID);

fileID = fopen('conjunctiveRates_test.bdat','wb');
fwrite(fileID,rates_conjunctive_time,'float32');
fclose(fileID);

fileID = fopen('ADNRates_test.bdat','wb');
...
if visual_scene
    fileID = fopen('LandmarkVisibility_test.bdat','wb');
...
%% Plot test rates
RSC_model_plot_v3('SO_test','simulation',1,time, timestep_size, conjunctive_cells,num_landmarks, hd_cells, 'save',...
